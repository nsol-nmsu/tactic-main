...
Consumer::RequestNext( void ) {
    // Make the request.
    auto request =
        std::make_shared<DataRequest>( m_contentName, m_nextSeg++ );
    request->SetNonce( m_nonceRng->GetInteger() );
    request->SetLifetime( m_interestLifetime );
    request->SetAuth( *m_authTag );
    request->SetAuthProb( 0 );
    request->SetRouteHash( 0 );
    request->SetNonce( m_nonceRng->GetInteger() );
    request->SetSignature( DUMMY_SIGNATURE );
    ...
    auto cb = std::bind( &Consumer::OnResponse, this, _1, _2 );
...
Consumer::RetransmitNext( void ) {
    ...
    if( !request->IsAuthRequest() )
        ((Request&)*request).SetAuth( *m_authTag );
...
void
Consumer::OnResponse( SPtr< PendingRequest >& p, Response& r ) {
